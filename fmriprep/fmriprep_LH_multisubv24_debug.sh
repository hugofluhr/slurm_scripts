...
# Use as:
# sbatch --array=1-$(( $(wc -l /home/hfluhr/data/learninghabits/dev/participants_remaining.tsv | cut -f1 -d' '))) scripts/fmriprep/fmriprep_LH_multisubv24_debug.sh
# sbatch --array=1-$(( $(wc -l /home/hfluhr/data/learninghabits/dev/dev_subset.tsv | cut -f1 -d' '))) scripts/fmriprep/fmriprep_LH_multisubv24_debug.sh
...
subID=$(sed -n "${SLURM_ARRAY_TASK_ID}p" ${SUBJ_LIST_DIR}/dev_subset.tsv)
SUBJECT="sub-${subID}"
echo "Processing the anatomical and functional workflows on subject: $SUBJECT"

# Paths
DATADIR="/home/hfluhr/data/learninghabits/sdc_test"
...
FMRIPREP_CMD="/data /out/fmriprep-24.0.1_sdc_test participant --participant-label $SUBJECT \
-w /work"
